...
from core.autocomplete import (
    ...
    response_trigger_autocomplete,
    response_followup_autocomplete,
)
        ...
        try:
            ...
            embed.add_field(name="__**Setting Commands**__",
                            ...
                            **/admin settings reset** - Reset the server settings\n
                            """,
                            inline=False)
            embed.add_field(name="__**Dynamic Response Commands**__",
                            value="""
                            **/admin response add_trigger** - Add a dynamic trigger\n
                            **/admin response update_trigger** - Update a dynamic trigger\n
                            **/admin response remove_trigger** - Delete a dynamic trigger\n
                            **/admin response add_channel/remove_channel** - Add/Remove channel restriction\n
                            **/admin response add_role/remove_role** - Add/Remove role restriction\n
                            **/admin response add_user/remove_user** - Add/Remove user restriction\n
                            **/admin response add_followup/remove_followup** - Add/Remove button followup\n
                            **/admin response list** - List all dynamic responses\n
                            """, inline=False)
        ...
        async with aiosqlite.connect(f"pathparser_{guild_id}.sqlite") as conn:
            ...
            try:
                ...
                if amount == 0 and misc_milestones == 0:
                    ...
                    await cursor.execute(
                        "Select True_Character_Name, Character_Name, Thread_ID, Level, Milestones, Tier, Trials, Personal_Cap, Region, Heroism, Hero_Points, Hero_Points_Max FROM Player_Characters where Character_Name = ? OR Nickname = ?",
                        (unidecode_name, unidecode_name))
                    player_info = await cursor.fetchone()
                    if not player_info:
                    ...
                    if player_info:
                        (true_character_name, character_name, thread_id, character_level, milestones, tier, trials,
                         personal_cap, region, heroism, hero_points, hero_points_max) = player_info
            ...
            try:
                ...
                if new_level < level_minmax[0]:
                    ...
                    if new_level_info:
                        ...
                        await config.config_cache.load_configurations(guild_id=guild_id)
                        await cursor.execute(
                            "SELECT True_Character_Name, Character_Name, Level, Milestones, Tier, Trials, personal_cap, Thread_ID, region, Player_ID, Heroism, Hero_Points, Hero_Points_Max FROM Player_Characters WHERE Milestones >= ?",
                            (minimum_milestones,))
                            ...
                            for idx, character in enumerate(characters_to_adjust):
                                ...
                                try:
                                    (true_character_name, character_name, character_level, milestones, tier, trials,
                                     personal_cap, thread_id, region, player_id, heroism, hero_points, hero_points_max) = character
                                    level_adjustment = await character_commands.level_calculation(
                                        character_name=character_name,
                                        level=character_level,
                                        guild=guild,
